...
def test_high_churn_high_spend():
    d = decide(tenure_months=36, monthly_spend=150, churn_risk=0.9, current_plan="Premium")
...
def test_medium_churn_short_tenure():
    d = decide(tenure_months=10, monthly_spend=40, churn_risk=0.55, current_plan="Starter")
...
def test_low_churn_basic_low_spend():
    d = decide(tenure_months=3, monthly_spend=25, churn_risk=0.2, current_plan="Starter")
    assert d.offer_name == "Mid-Tier Bundle"
    assert d.discount_pct == 5


def test_low_churn_premium_plan():
    d = decide(tenure_months=24, monthly_spend=120, churn_risk=0.1, current_plan="Premium")
    assert d.offer_name == "Add-On Services Pack"
    assert d.discount_pct == 0


def test_default_unknown_plan():
    d = decide(tenure_months=12, monthly_spend=50, churn_risk=0.2, current_plan="Legacy Gold")
