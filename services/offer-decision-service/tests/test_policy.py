...
class TestHighChurnHighValue:
    ...
    def test_boundary_churn_0_7_spend_80(self):
        ...
        d = decide(
            tenure_months=12, avg_monthly_spend=80, churn_risk=0.7, current_plan="Basic"
        )
        assert d.offer_name == "Premium Loyalty Bundle"
        assert d.discount_pct == 25
    ...
    def test_rule1_takes_priority_over_new_subscriber(self):
        ...
        d = decide(
            tenure_months=2, avg_monthly_spend=90, churn_risk=0.75, current_plan="Basic"
        )
        assert d.offer_name == "Premium Loyalty Bundle"
        assert d.discount_pct == 25
    ...
    def test_rule1_takes_priority_over_upsell(self):
        ...
        d = decide(
            tenure_months=30, avg_monthly_spend=85, churn_risk=0.7, current_plan="Basic"
        )
        assert d.offer_name == "Premium Loyalty Bundle"
        assert d.discount_pct == 25
...
class TestHighChurnLowValue:
    ...
    def test_zero_spend_high_churn(self):
        d = decide(
            tenure_months=6, avg_monthly_spend=0, churn_risk=0.8, current_plan="Basic"
        )
        assert d.offer_name == "Loyalty Saver Plan"
        assert d.discount_pct == 15
    ...
    def test_churn_exactly_0_7_spend_zero(self):
        d = decide(
            tenure_months=0, avg_monthly_spend=0, churn_risk=0.7, current_plan="Basic"
        )
        assert d.offer_name == "Loyalty Saver Plan"
        assert d.discount_pct == 15
...
class TestLongTenureLowSpendUpsell:
    ...
    def test_boundary_tenure_24_spend_49(self):
        d = decide(
            tenure_months=24, avg_monthly_spend=49, churn_risk=0.2, current_plan="Basic"
        )
        assert d.offer_name == "Upsell to Standard Plus"
        assert d.discount_pct == 10
    ...
    def test_boundary_tenure_24_spend_0(self):
        d = decide(
            tenure_months=24, avg_monthly_spend=0, churn_risk=0.1, current_plan="Basic"
        )
        assert d.offer_name == "Upsell to Standard Plus"
        assert d.discount_pct == 10
    ...
    def test_spend_exactly_50_does_not_match(self):
        ...
        d = decide(
            tenure_months=36, avg_monthly_spend=50, churn_risk=0.3, current_plan="Basic"
        )
    ...
    def test_tenure_23_does_not_match(self):
        ...
        d = decide(
            tenure_months=23, avg_monthly_spend=30, churn_risk=0.3, current_plan="Basic"
        )
        assert d.offer_name != "Upsell to Standard Plus"
...
class TestNewSubscriberWelcome:
    ...
    def test_zero_tenure(self):
        d = decide(
            tenure_months=0, avg_monthly_spend=50, churn_risk=0.1, current_plan="Basic"
        )
        assert d.offer_name == "Welcome Booster Pack"
        assert d.discount_pct == 20
    ...
    def test_new_subscriber_high_churn_goes_to_rule1(self):
        ...
        d = decide(
            tenure_months=1, avg_monthly_spend=100, churn_risk=0.9, current_plan="Basic"
        )
        assert d.offer_name == "Premium Loyalty Bundle"

    def test_explanation_mentions_tenure(self):
        d = decide(
            tenure_months=3, avg_monthly_spend=60, churn_risk=0.1, current_plan="Basic"
        )
...
class TestDefaultStandardRenewal:
    ...
    def test_mid_tenure_mid_spend_low_churn(self):
        d = decide(
            tenure_months=12, avg_monthly_spend=60, churn_risk=0.5, current_plan="Basic"
        )
        assert d.offer_name == "Standard Renewal"
        assert d.discount_pct == 0
    ...
    def test_tenure_6_spend_50_churn_below_0_7(self):
        ...
        d = decide(
            tenure_months=6, avg_monthly_spend=50, churn_risk=0.69, current_plan="Basic"
        )
        assert d.offer_name == "Standard Renewal"
        assert d.discount_pct == 0
...
class TestExtremeValues:
    ...
    def test_zero_everything(self):
        d = decide(
            tenure_months=0, avg_monthly_spend=0, churn_risk=0, current_plan="Basic"
        )
    ...
    def test_churn_exactly_zero(self):
        d = decide(
            tenure_months=12, avg_monthly_spend=60, churn_risk=0.0, current_plan="Basic"
        )
        assert d.offer_name == "Standard Renewal"
    ...
    def test_empty_plan_string(self):
        ...
        d = decide(
            tenure_months=12, avg_monthly_spend=60, churn_risk=0.3, current_plan=""
        )
        assert d.offer_name == "Standard Renewal"
...
class TestRulePriority:
    ...
    def test_rule2_beats_rule3(self):
        ...
        d = decide(
            tenure_months=36, avg_monthly_spend=30, churn_risk=0.8, current_plan="Basic"
        )
        assert d.offer_name == "Loyalty Saver Plan"
    ...
    def test_rule5_beats_default(self):
        ...
        d = decide(
            tenure_months=4, avg_monthly_spend=55, churn_risk=0.4, current_plan="Basic"
        )
        assert d.offer_name == "Welcome Booster Pack"
